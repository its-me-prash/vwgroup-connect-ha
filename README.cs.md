...
- **GPS sledovač polohy**, přes 100 entit napříč několika platformami, přes 20 volání služeb, více vozidel na účet, názvy entit ve **12 jazycích**.
...
| **VW USA/CA** | ✅ Obousměrné | ✅ Plné | cloud VW NA (vyžaduje volbu země USA/CA + S-PIN). ⚠️ Vlastní přihlašovací služba VW aktuálně odpovídá na část kanadských přihlášení chybou serveru: to je výpadek na straně VW, ne špatné heslo ([#915](https://github.com/its-me-prash/vwgroup-connect-ha/issues/915)) |
...
- **Severní Amerika je experimentální.** Přihlášení **Audi USA/CA** je zapojené, ale nikdy nebylo potvrzeno na skutečném účtu ([#13](https://github.com/its-me-prash/vwgroup-connect-ha/issues/13)). **VW USA/CA** funguje, ale vlastní přihlašovací služba VW aktuálně vrací u části kanadských účtů chyby serveru ([#915](https://github.com/its-me-prash/vwgroup-connect-ha/issues/915)). Nevybírejte si tuhle integraci pro severoamerické auto s očekáváním, že prostě pojede.
...
interval skenování (dostupný i živě jako posuvník intervalu dotazování), S-PIN (a k tomu S-PIN pro jednotlivá vozidla, pokud je na účtu víc než jedno auto), zpětné geokódování, **režim jen pro čtení**, vynucenou klimatizaci PPE (Audi), přepínače push (MQTT/FCM/Audi-VW, všechny volitelné, v betě a ve výchozím stavu vypnuté), přepsání client-id, **`eu_data_act_auto_kickoff`** (výchozí zapnuto), skrytí prázdných entit (výchozí zapnuto), **ABRP** (zapnutí + api_key + uživatelský token, validováno jako pár), plus **přidání / odebrání** doplňkových čtecích kanálů: `volkswagen.de` (beta), portál EU Data Act a **Tibber**.
