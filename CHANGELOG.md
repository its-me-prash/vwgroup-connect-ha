...
- **Audi PPE (S6 / Q6 e-tron) windows and doors now read their real open/closed state.** On PPE cars each access-status entry ships its `status` as a list of plain strings (`["open"]`) rather than objects (`[{"value": "open"}]`), and we only read the object form — so a physically-open window or door parsed to *closed*, with an empty `windows_individual` / `windows_position` (#1279, @peterbauer1709). It now reads either shape, and also surfaces each window's opening percentage (`windowOpen_pct`).
- **The "Request historical export" button is reachable again, and no longer times out a real request early (#923, @naked-head).** The wedge-guard refused to submit a one-time export whenever the 15-minute continuous feed was active — but that feed is active for essentially everyone via auto-kickoff, so the button always refused. Field testing showed the portal actually allows a one-time export *alongside* the continuous feed (which keeps publishing); it only enforces one *pending* request at a time. The guard now refuses only while your own one-time export is still pending, and the client-side deadline is raised from 26h to 72h since a request can legitimately still be gathering well past a day.
...
### Fixed
- **Audi cars now show their model — the "Audi (2021)" S6 becomes "Audi S6 Avant TDI quattro tiptronic".** An Audi's model name (and its render images) come from a graphql `userVehicles` query, where the model lives in the *localised* `media.shortName`/`longName`. Those are only returned when the request carries a locale — our request omitted `Accept-Language` + `X-User-Country`, so the backend answered with the model **year** but a null `media`, and every affected car fell back to a bare "Audi (2021)". A live A/B check (same account, same token, same query) confirmed it exactly: no locale headers → all model names null; `Accept-Language: de-DE` + `X-User-Country: DE` → full names for all cars. The request now always sends a valid locale (taken from the account's country, defaulting safely), so the model appears. The list is also read from the **myAudi app-API first** (the source the classic myAudi clients use, with the web-proxy as a fallback) and used to **discover vehicles**, so a car the modern vehicle-list omits still appears with its model and year. The render-image entities were also given clean, consistent, localized names.
...
### Added
- **Companion phone: a third way to connect, where the phone calls Home Assistant (#968).** Until now HA had to reach *into* the phone — ADB over TCP, or the ADB Bridge add-on doing the same — and that direction is where setups get stuck: Android 11+ wireless debugging has to be paired by hand and switches itself off, phone IPs move under DHCP, and guest/IoT Wi-Fi with client isolation blocks it outright. A companion **agent app** on the phone can now make an ordinary outbound HTTPS long-poll to Home Assistant instead, taking its next command from the response. Nothing has to reach the phone, so NAT, changing IPs, client isolation and wireless debugging all stop mattering, and no add-on is needed. Tick **"Use the companion agent app"** at setup and paste the token the agent generated; the address field stays empty. The endpoint is gated by that random per-vehicle token (compared in constant time, redacted in diagnostics), a token resolves to exactly one vehicle and a duplicate one binds to none, and the phone is never handed anything internal to HA. What crosses the wire is a short fixed list of screen verbs — dump the visible screen, tap, back, foreground, app version — never a shell. The protocol is specified in [`docs/COMPANION_AGENT.md`](docs/COMPANION_AGENT.md); the phone-side agent app is a separate artifact and is not part of this repository yet, so this ships as the Home Assistant half.
- **Companion: parking position, read from the app's own share link (#923, #968).** A VW EU car read through the EU Data Act portal has no position data point at all, and the app draws the parked car as a map with no coordinate text — so both existing paths are structurally blind to where the car is. The app's share sheet is not: sharing the parking spot renders a Google Maps link whose preview text carries the coordinates. The companion channel can now walk to it, read the coordinates and come straight back. Opt-in, off by default, and read-only — nothing is ever sent from the share sheet.
- **Companion: odometer, next service, charge limit, target and outside temperature from the detail screens (#968).** We Connect 4.3.2 dropped the total odometer from the overview (it shows a "Driving data" tile with the last trip instead), so mileage and the service countdown are now read from the Vehicle Health screen, the charge limit from vehicle Settings, and the target/outside temperature from the climate detail. On the MEB layout the first two sit below the fold, so the walk scrolls the overview first — measured in fractions of whatever display the phone reports, not in pixels. Each path is its own opt-in and each declares how far back it walks, so enabling one never silently starts a deeper one.
- **Companion: the walk comes back through the app's own close buttons, not Android's BACK (#968).** Global BACK is not bounded by the app: from a shallow navigation stack it leaves it entirely, and the next poll then finds a launcher instead of a car. The nav walk now taps We Connect's own up/close control wherever the screen offers one, falls back to BACK only when it does not, and stops as soon as the overview is back rather than pressing a fixed number of times. It also waits for a screen to stop changing before the next step reads it, instead of reading a half-built one.

### Fixed
- **Companion: a car that is plugged in but not charging is no longer read as charging (#968).** We Connect 4.3.2 merges state and value into a single narration ("Charging status. Battery charge level: 79 per cent. Charging stopped") — and the word "Charging" is in that sentence either way, so the charging flag came back true regardless and the charging state sensor showed the whole sentence. Both are now parsed out of the merged label properly, on English and German builds.
- **Companion: a grouped-thousands odometer with a comma no longer reads as its first two digits (#968).** On an English build the Vehicle Health report writes the mileage as `27,886 km`; the thousands separator was only recognised as a space or a dot, so that odometer came back as 27 km. A three-digit group after a comma now collapses the same way (a decimal comma still cannot match, since a group has to be exactly three digits).
- **Companion: a greyed-out control is no longer treated as a tap target (#968).** The accessibility tree's `enabled` flag was dropped when parsing a screen, so a disabled button counted as tappable — the tap does nothing and the walk then reads the wrong screen. `enabled`, `checkable` and `checked` are now kept (the last two are what a settings toggle's state actually lives in).
- **Companion: the two advanced toggles now actually take effect when changed in Options (#968).** "Read charge target" and "wake screen to poll" were read from the entry's setup data only, while the Options flow writes to the entry's options — so a user who ticked either one after setup saw nothing happen, with no error to explain why. Both now read options first and fall back to the setup value.
- **A car that Home Assistant disabled for its own reasons no longer silently drops out of polling (#1234).** The poll loop skips a car whose device *you* disabled, so a deactivated car stops consuming the daily request budget — but it was also skipping a device HA had disabled for its own internal reasons, which could strand one car on a multi-car account out of the poll rotation, silently, with a restart not bringing it back. It now only skips a car you disabled yourself. Thanks @eddieari.
