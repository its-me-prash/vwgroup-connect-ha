...
- **Unified `vag_connect.execute_vehicle_action` service**. One service with a dropdown of 14 actions (lock, unlock, start/stop climatisation, start/stop charging, flash lights, start/stop window heating, wake vehicle, start/stop aux heating, start/stop ventilation) instead of users hunting through 14 separate services in the Lovelace service picker. Device-targeted via `device_id` so HA resolves the vehicle for the user. All existing per-action services (vag_connect.lock, vag_connect.start_climatisation, etc) keep working unchanged, so existing automations are NOT broken. Pattern observed in arjenvrh/audi_connect_ha v2.1.0; independent implementation.
...
- **Refuel-trip aggregator** — 9 new sensors that track consumption since the last tank fill or charge session: `refuel_trip_distance_km`, `refuel_trip_duration_min`, `refuel_trip_avg_speed_kmh`, `refuel_trip_avg_fuel_consumption_l_100km`, `refuel_trip_avg_electric_consumption_kwh_100km`, `refuel_trip_total_fuel_consumption_l`, `refuel_trip_total_electric_consumption_kwh`, `refuel_trip_recuperation_kwh`, `refuel_trip_timestamp`. Sourced from CARIAD BFF `tripstatistics?type=cyclic`. The total-consumption-per-tank/charge values are Energy-Dashboard-friendly and were missing in the HA-VAG ecosystem until now. Pattern observed in volkswagencarnet's `TRIP_REFUEL` constant; independent parser implementation.
- **Structured warning-lights parser** for SEAT/CUPRA via the dedicated `/v3/vehicles/{vin}/warninglights` OLA endpoint. Replaces the silenced `vehicleHealthWarnings.warningLights.error` envelope path that triggered repeated Scout reports (#384, #389). Per-category aggregation: `warning_engine`, `warning_brakes`, `warning_tyre`, `warning_oil` booleans plus a human-readable `warning_messages` summary and `warning_count` total. Endpoint reference: Timwun/Cupra-WeConnect-Bruno-Collection.
- **Settable battery-care config** for SEAT/CUPRA. New `command_set_battery_care(enabled)` and `command_set_battery_care_target(percent)` actions hit `/v1/vehicles/{vin}/charging/battery-care` and `/target`. Read side adds `battery_care_target_soc_pct` sensor populated from the GET endpoint. The bool `battery_care` field gained in v2.8.1 was read-only; this completes the surface as user-settable.
- **Audi `plug_led_color` parser** wired up. The field has existed on `VehicleData` since v2.0 but no parser populated it; now parsed from the CARIAD BFF response with three field-name variants tried defensively. Q6/A6 e-tron PPE platform reports the LED color (None / Red / Green / Blue); older Audi models leave the field None and the existing sensor stays hidden.
- **`actual_charge_rate_kw` real-time sensor** for Audi + VW EU. Distinct from the existing averaged `charging_rate_kmh`. Only populated by firmware that exposes a separate instant rate (Q6 e-tron observed; older models report only the average).
- **Rich climate-start service** `vag_connect.start_climate_control` for Audi + VW EU. Surfaces the full CARIAD BFF `/climatisation/start` payload to HA automations: per-seat heating (front-left, front-right, rear-left, rear-right), glass heating, climatisation mode (comfort or economy), continue-while-unlocked toggle and target temperature in one call. Uses a device selector so dashboards can target a vehicle by id instead of carrying VINs around. Other brands fall through to the basic `start_climatisation` service because the OLA / mysmob / PPA backends reject the extra payload fields. Coordinator-level optimistic UI mirrors the basic-start path so the climate sensors flip immediately.
- **Charging statistics endpoint** for SEAT/CUPRA. New parser polls `/charging_statistics` and `/charging_statistics/{vin}/power-curve` on the separate `prod.emea.mobile.charging.cariad.digital` host (same Bearer token, different vhost). Populates the cross-brand `total_charged_energy_kwh`, `last_charging_session_kwh`, `last_charging_session_duration_min`, `last_charging_session_start`, `last_charging_session_current_type`, and `recent_charging_sessions` surface that has so far been Skoda-only (v1.15.0 #35). Adds a new `last_charging_power_curve_points` diagnostic sensor whose state is the per-sample COUNT for the most recent session, full sample list in attributes, suitable for Lovelace cards that graph kW over time / SoC for the last DC fast-charge. Soft-fails to empty on 401, 403, 404 so older firmware and accounts without an active charging subscription stay clean. Endpoint reference: Timwun/Cupra-WeConnect-Bruno-Collection. Closes a feature gap that has had no Energy-Dashboard story for SEAT/CUPRA users before now.
- **SEAT/CUPRA OLA endpoint parity (Group B)**. Seven new OLA endpoints wired into `seat_cupra.get_status`: `/v1/vehicles/{vin}/notifications` populates `notifications_count` plus `last_notification_subject` and `last_notification_severity` as three diagnostic sensors; `/v1/vehicles/{vin}/permissions` populates `permission_is_owner` and `permission_can_command` as two diagnostic binary sensors (parsed from either an explicit `role` field or the `permissions[].name` membership defensively); `/v1/vehicles/{vin}/measurements/engines` populates the new `engine_oil_temperature_c` and `engine_coolant_temperature_c` temperature sensors with Kelvin-to-Celsius conversion when the backend ships Kelvin; `/v1/vehicles/{vin}/charging/profiles` reuses the existing Skoda profile surface (`active_charging_profile_name`, `active_charging_profile_target_soc_pct`, `next_charging_time`, `charging_profiles_count`, `charging_profiles`) so the same sensors light up across both brands; `/v1/vehicles/{vin}/charging/modes` populates `available_charge_modes` which surfaces as an attribute on the existing `charging_preferred_mode` sensor; `POST /v1/vehicles/{vin}/charging/actions/update-settings` is exposed as the new `vag_connect.update_charging_settings` service accepting optional `target_soc`, `max_charge_current` and `auto_unlock_charge` payload fields with at-least-one-required validation; `/v1/charging/points` extends `SeatCupraClient.find_charging_stations` to the public OLA POI catalog so the existing `vag_connect.find_charging_stations` service now works on SEAT/CUPRA in addition to Audi and VW EU. All five new fetch endpoints soft-fail to `{}` on 401 / 403 / 404 so older firmware stays clean. All new sensors and binary sensors phantom-protected via `_DATA_PRESENT_REQUIRED`. Endpoint reference: Timwun/Cupra-WeConnect-Bruno-Collection.

#### VW EU field parity (Group A)

Closes the 10 conceptual VW EU competitor-library gaps surfaced during the 2026-06-02 scan (`docs/v2.10.0_GAP_CLOSURE_STRATEGY.md`, Group A). All fields default to None on `VehicleData`, parse defensively from 2-3 field-name variants per the existing convention, and are phantom-protected via `_DATA_PRESENT_REQUIRED` so cars without the underlying CARIAD BFF leaves stay clean.

- **HV battery min / max temperature** (`hv_battery_min_temperature_c`, `hv_battery_max_temperature_c`). Separate min/max from `charging.batteryStatus.value.{minTemperature_K,maxTemperature_K}`, in addition to the existing `battery_temp` / `battery_temp_max` pair on the measurements block. Kelvin-to-Celsius only when the raw value is greater than 200 (heuristic to distinguish Kelvin from already-Celsius readings).
- **Max AC charging current** split into setting vs deliverable (`charge_max_ac_setting`, `charge_max_ac_ampere`). Distinguishes the user-requested cap from the actual deliverable amperage the wallbox plus cable supports.
- **Born MY24+ AC connector auto-release** (`auto_release_ac_connector` bool, `auto_release_ac_connector_state` enum). Boolean flag plus state enum (typical values `IDLE`, `RELEASING`).
- **Optimised battery use** (`optimised_battery_use`). Born / ID.x setting distinct from `battery_care` that limits charging dynamics and thermal pre-conditioning to preserve cell longevity.
- **Active ventilation** (`active_ventilation_state`, `active_ventilation_remaining_time_min`). Cabin air-circulation separate from the climatisation block. State enum (`off` / `running` / `finished`) plus countdown minutes.
- **Rear sunroof and Cabrio roof cover** (`sunroof_rear_closed`, `roof_cover_closed`). Two new window-class binary sensors with invert semantics so the HA UI shows "On = open" while the underlying field stores "True = closed".
- **12V health bucket** (`connection_state_battery_power_level`). Enum surface (`low` / `normal` / `high`) from `connectionStatus.batteryPowerLevel` or `vehicleHealthInspection.value.battery12VLevel`, companion to the existing `connection_battery_power_level` field which reads the same signal from a different parent block on other firmware shapes.
- **Last trip totals** (`last_trip_total_fuel_consumption_l`, `last_trip_total_electric_consumption_kwh`). Absolute per-trip totals alongside the existing per-100km averages. Backend totals win when present; otherwise derived from `avg * distance / 100`. A backend-supplied total is never overwritten.

#### VW NA endpoint parity (Group C)

- **VW NA subscription + capability sensors** wired up. New `get_subscription_privileges` helper polls `GET /rrs/v1/privileges/user/{user_id}/vehicle/{uuid}` and populates the cross-brand `subscription_active`, `subscription_expiry_at`, `subscription_days_remaining`, and `capabilities_count` fields for Cox-backend VW NA accounts. Pre-v2.10.0 those four sensors were SEAT/CUPRA + VW EU only; VW NA now reports them too where the Cox privileges endpoint exposes them. Soft-fails to empty dict on 401, 403, 404, non-dict response, or when the user_id could not be captured, so older firmware and trial-expired accounts leave the sensors hidden via the existing `_DATA_PRESENT_REQUIRED` gate. Endpoint reference: zackcornelius/CarConnectivity-connector-volkswagen-na.
- **VW NA two-step SPIN flow** for privileged actions. NA uses a different SPIN protocol from EU: clients must `POST /ss/v1/user/{user_id}/challenge` to obtain a per-action nonce, compute `SHA1(SPIN + nonce).hexdigest().upper()` as the response, then `POST /ss/v1/user/{user_id}/spin` to receive a short-lived session token. That token is presented as an `X-Spin-Session` header on the action POST. Pre-v2.10.0 the unlock command put the raw SPIN into the action body, which Cox post-2024 firmware rejects. The legacy in-body SPIN path is preserved as a fallback so existing accounts on older firmware keep working.
- **VW NA NA-specific lock/unlock endpoint**. `POST /lockunlock/v1/vehicle/{uuid}` is the modern Cox path for both lock and unlock actions. Falls back to the legacy `POST /ev/v1/vehicle/{uuid}/lock` on a 404 response so installs on pre-2024 Cox firmware keep working through the transition window.
- **VW NA dedicated climatisation + window heating paths**. `command_start_climate`, `command_stop_climate`, `command_start_window_heating`, and `command_stop_window_heating` now hit the NA-specific `pretripclimate` and `pretripclimate/windowheating` endpoints first, falling back to the EU-style `climatisation` aliases on a 404 for older firmware. Same defensive A/B fallback pattern proven in the v1.17.1 SEAT/CUPRA Bruno-Collection work. Endpoint reference: zackcornelius/CarConnectivity-connector-volkswagen-na.
