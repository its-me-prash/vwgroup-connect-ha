...
- **Companion: odometer, next service, target and outside temperature from the detail screens (#968).** We Connect 4.3.2 dropped the total odometer from the overview (it shows a "Driving data" tile with the last trip instead), so mileage and the service countdown are now read from the Vehicle Health screen, and the target/outside temperature from the climate detail. Each path is its own opt-in and each declares how far back it walks, so enabling one never silently starts a deeper one.
