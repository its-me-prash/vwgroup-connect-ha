...
### Added
- **Audi plug&play (OBD dongle) now surfaces the real vehicle identity and full factory master-data.** The dongle's carport record gives the proper "ab Haus" model designation in the same style as a connected car — e.g. **"Audi" + "A5 Coupé TDI"** (the body/chassis form is decoded from the Audi sales-type code, and the power is no longer crammed into the name) — plus a clean manufacturer (**Audi**, not "Audi_Acpp"). Adds a **fuel-level sensor in litres** and, as diagnostic sensors, one consolidated **engine-power** figure (**"176 kW / 239 PS"**), **displacement, torque, cylinder count, engine code, fuel type, transmission** (with its gearbox code, e.g. "Manual (Code: KMU)") and **exterior + interior colour** — all localized in all 12 languages — plus a **"Data as of" timestamp** so you can see how fresh the dongle's last sync is (a plugged-in dongle only refreshes when the car is driven). The real **first-registration / delivery date** and **warranty-end date** become service-calendar events. (acpp exposes no vehicle image — verified: it has no image endpoint, and the myAudi render resolvers reject the dongle client — and no reliable inspection date, so those are deliberately omitted.)
...
- **Audi plug&play master-data came back empty on a live read.** The carport endpoint reads its language from `Accept-Language` and rejects a multi-value / q-weighted value (`de-DE, en-US;q=0.9`) with HTTP 400 while accepting a bare `de-DE` — so every carport-derived field (model, colours, power, dates) silently came back empty against a real car even though the mocked tests were green. Now sends a single tag, and a non-200 sub-resource is logged in debug instead of being dropped silently, so a future live-only regression is visible.
