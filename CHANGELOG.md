...
### Added
- **Audi plug&play (OBD dongle) now surfaces the real vehicle identity and full factory master-data.** The dongle's carport record gives the proper "ab Haus" model designation in the same style as a connected car — e.g. **"Audi" + "A5 Coupé TDI"** (the body/chassis form is decoded from the Audi sales-type code, and the power is no longer crammed into the name) — plus a clean manufacturer (**Audi**, not "Audi_Acpp"). Adds a **fuel-level sensor in litres** and, as diagnostic sensors, one consolidated **engine-power** figure (**"176 kW / 239 PS"**), **displacement, torque, cylinder count, engine code, fuel type, transmission** and **exterior + interior colour** — all localized in all 12 languages. The real **first-registration / delivery date** and **warranty-end date** become service-calendar events. (acpp exposes no vehicle image — verified: it has no image endpoint, and the myAudi render resolvers reject the dongle client — and no reliable inspection date, so those are deliberately omitted.)
