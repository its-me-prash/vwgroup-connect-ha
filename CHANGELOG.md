...
- **Škoda climate / window-heating / ventilation switches now use the app's real capability names.** The capability ids for these were educated guesses (`air-conditioning`, `window-heating`) that never match what the MySkoda backend actually sends (`AIR_CONDITIONING`, `WINDOW_HEATING`, `ACTIVE_VENTILATION`, `AUXILIARY_HEATING` — verified against the 8.15.0 app). Harmless while a car's capability list is empty, but it would have wrongly hidden those switches the moment the list populated. Now corrected. (Charging and lock ids are left as-is on purpose until a live capabilities sample confirms them — a wrong guess there could hide charging on a MEB Škoda.)
