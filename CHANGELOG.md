...
- **Volkswagen US/Canada: the climate temperature and the door-lock state now show up.** Both were in the data the car sent all along. The target temperature arrives wrapped together with its unit (70 °F, for example) and the reader only understood a plain number, so it came out empty — the same wrapped shape this integration itself sends when you set the temperature. The lock state is reported in up to three different places depending on the car, and only one of them was being read, so the lock showed Unknown even with every door reporting locked. Thanks @fg877khkv8-maker for the diagnostics that showed exactly this.
