...
### Fixed
- **Audi cars now show their model, and cars the modern list left out now appear (the "Audi (2021)" S6).** An Audi's model name (and its render images) come from a graphql `userVehicles` query. We read it from the `www.audi.de` web-proxy, which rejects some accounts' tokens outright — so those cars fell back to a bare "Audi (2021)" with no model, and any car the modern vehicle-list didn't return was simply invisible. It now reads that list from the **myAudi app-API first** (the source the classic myAudi clients use) and falls back to the web-proxy, and it uses the list to also **discover vehicles** — so a car the modern list omits appears with its model and year carried from the list. The render-image entities were also given clean, consistent, localized names.
