...
- **App Atlas covers all 7 brands** — APKCombo added as 3rd-tier scraper fallback closes the VW EU gap. Cross-brand visibility now 7/7 (`docs/research/app-atlas/_summary.md`).
- **Community audit pack** — three new research docs in `docs/research/`: (1) deep-audit of daernsinstantfortress's `weconnect_cupra` library showing 9 endpoint differences vs ours + architectural notes, (2) cross-project scan tracking mitch-dc's archival (2025-10-29 — major reference died), audi_connect_ha maintenance-only status, and skodaconnect's recent ACTIVE_VENTILATION + Firebase-MQTTv5 work, (3) consolidated v2.5.0 action plan with ~10h of follow-up items prioritised. No integration code touched in this round.
...
### Fixed
- **VW EU was silently impersonating the Audi app via the `x-android-package-name` token header** (latent bug — was not triggering enforcement yet, but our own atlas profile flagged this as wrong since 2026-05-29). `idk.py`'s `_cariad_token_headers()` hardcoded `de.myaudi.mobile.assistant` for *every* brand including VW EU. Pre-v2.5.11 this passed because Azure WAF does not currently enforce brand-consistency between OAuth `client_id` and `x-android-package-name`. If they ever enable that anomaly-score check, VW EU would have broken instantly. v2.5.11 wires a per-brand `BrandConfig.android_package_name` field (default empty for backward-compat) and threads it through to both call sites in `idk.py` (token exchange + refresh). Values now match the atlas profile: VW=`de.volkswagen.weconnect`, Audi=`de.myaudi.mobile.assistant`, SEAT=`com.seat.myseat.ola`, CUPRA=`com.cupra.mycupra`, Skoda=`cz.skodaauto.myskoda`. No behavioural change for users today, but future-WAF-rule-resilient.

### Added
- **Audi market-config dynamic discovery** — replicates `audi_connect_ha` PR #736's "killer feature" that detected the 2026-05-28 WAF migration *automatically*, before their devs even read the GitHub thread. New module `cariad/auth/_audi_market_config.py` fetches the Audi app's startup config endpoint (`https://content.app.my.audi.com/service/mobileapp/configurations/market/{country}/{language}?v=4.23.1`) and caches it for 24h. Three resilience-critical keys flow through: `idkLoginServiceConfigurationURLProduction` (becomes our token URL via `derive_token_url_from_market_config()`), `idkClientIDAndroidLive` (becomes the first candidate in our OAuth client_id chain), and `mbbOAuthBaseURLLive` (for the legacy fs-car path). Plugs into `AuthConfigResolver.token_url()` between OIDC discovery (1h TTL) and APK cache, and into `oauth_client_id_chain()` as the first candidate. Best-effort with hard fallback — a failed/timeout/malformed response leaves the existing chain (OIDC discovery → APK cache → hardcoded constants) intact. **Future WAF migrations that flip the path don't need a code release any more — the next HA restart picks up the new URL automatically.**
- **evcc-derived alternate OAuth `client_id` for Audi**: `f4d0934f-32bf-4ce4-b3c4-699a7049ad26@apps_vw-dilab_com`. Spotted in evcc's `vehicle/audi/` package post-2026-05-28 — NOT in `audi_connect_ha`'s tree, so this is a separately-discovered candidate from independent reverse-engineering. Added to `_ALTERNATE_CLIENT_IDS["audi"]` after the existing `16dd7960-...` candidate. Will only be tried as a fallback when the primary `09b6cbec-...` returns 4xx.

### Changed
- **`c95f4fd2` qmauth prior pair marked CONFIRMED DEPRECATED** in `vw_group_auth_profile.json`. Cross-source verification on 2026-05-30: evcc PR #30292 explicitly removed this pair on 2026-05-28; audi_connect_ha master branch (PR #736 + history) has *never* contained `c95f4fd2` — it was only ever `01da27b0` since 2022-12-20. We keep `c95f4fd2` in the fallback chain as a last-resort for the unlikely VW-rollback scenario, but `AuthConfigResolver.qmauth_chain()` now logs a one-shot `_LOGGER.info()` the first time the deprecated pair is added to a chain. If you ever see that log followed by a "fallback attempt succeeded" line, please file an issue — that means VW unexpectedly rolled back the 2026-05-28 rotation.
- **Atlas profile (`docs/research/app-atlas/vw_group_auth_profile.json`)** — extensively annotated with `_v2511_status` fields per brand documenting the brand-impersonation fix, the market-config discovery wiring, and the new deprecation status for the prior qmauth pair. Atlas is now an accurate description of what the v2.5.11 code does, not what we *think* it should do.

### Tests
- `tests/test_v2511_auth_hardening.py` — 8 new unit tests covering all three patches. Pure-data (no HA dep): BrandConfig field existence + per-brand values, `_cariad_token_headers()` parameter wiring, market-config URL derivation (3 variants), cache-empty default, alternates chain inclusion, deduplication contract, deprecation-flag visibility.

### Notes
- **What this is NOT**: there is no user-visible regression mitigation in v2.5.11. The 2026-05-28 Azure WAF migration was already fully addressed by v2.5.4 (emergency endpoint fix) + v2.5.5–v2.5.7 (resilience layers). Users who are working today on v2.5.7+ keep working on v2.5.11. The value of this release is in reducing the *next* incident's MTTR — when VW rotates again (and they will), this release means we (a) survive the brand-consistency check if WAF gets stricter, (b) auto-discover the new URL via the Audi market-config layer, and (c) have one more alternate client_id to fall back to before raising a credentials error.
- **What audi_connect_ha does differently**: they send only 5 headers on token requests (no `x-platform`, `x-android-package-name`, `x-assertion`); we send all 8. Both work as of 2026-05-30. We keep the superset for now — it mimics the official app more closely and is the more anomaly-score-resilient setup if VW tightens WAF rules. Documented in `_cariad_token_headers()` docstring for future minimisation if/when WAF behaviour stabilises.
