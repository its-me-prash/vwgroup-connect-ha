...
<p align="center">
  <strong>Une seule intégration Home Assistant pour les marques du groupe Volkswagen — Audi · Volkswagen · Škoda · SEAT · CUPRA · Porsche · VW US/Canada · Bentley</strong><br>
  <em>Accès direct à l'API, multicanal avec bascule automatique, sans middleware.</em>
</p>
...
**VW Group Connect est une intégration [Home Assistant](https://www.home-assistant.io) qui amène les données et le contrôle des voitures connectées dans votre maison intelligente, pour les marques du groupe Volkswagen — Volkswagen, Audi, Škoda, SEAT, CUPRA, Porsche, VW US/Canada et Bentley — à partir d'une seule entrée de configuration.**

Elle expose l'état de la batterie et de la charge, l'autonomie, le compteur kilométrique, la climatisation, les portes et les vitres, la localisation et bien plus encore, et — là où le backend de la marque le permet encore — envoie des commandes à distance telles que le verrouillage/déverrouillage, le contrôle de la climatisation et de la charge. Pour continuer à fonctionner malgré les changements d'API de Volkswagen en 2026, elle parle **plusieurs canaux et bascule automatiquement** lorsque l'un d'eux est bloqué : les backends natifs des marques, le portail de données véhicule **EU Data Act** en lecture seule, un canal web `volkswagen.de` activable à la demande, et une connexion **sans mot de passe** durable pour les véhicules Car-Net plus anciens. Elle tourne sans souci **aux côtés d'[evcc](https://evcc.io)** et ne nécessite **aucune dépendance PyPI**.
...
- **8 marques sélectionnables du groupe Volkswagen** dans une seule intégration — Audi, Volkswagen EU, Škoda, SEAT, CUPRA, VW US/Canada, Porsche et Bentley.
- **Compatible Porsche** — Porsche s'appuie sur son propre backend *Porsche Connect*, **pas** sur le portail EU Data Act. Le chemin par le portail *exclut* structurellement Porsche, si bien que les outils qui ne reposent que sur le portail ne pourront jamais le couvrir ; cette intégration, si.
- **Contrôle bidirectionnel là où le backend de la marque le permet** — verrouillage/déverrouillage, climatisation, charge, SoC cible. Découvrez quelles marques disposent d'un vrai support des commandes dans le tableau ci-dessous ; VW EU est en lecture seule par défaut (voir la note honnête à ce sujet).
- **Option de connexion sans mot de passe** (navigateur/code d'appareil) pour Audi/Škoda/SEAT/CUPRA — aucun mot de passe stocké dans Home Assistant.
- **Multicanal avec bascule automatique** — backend natif de la marque → portail EU Data Act → web vw.de activable à la demande → Car-Net durable. Si un canal tombe, vos données ne s'éteignent pas pour autant.
- **Résilient par conception** — conserve les dernières valeurs connues pendant les pannes du portail, filtre les fausses sentinelles « aucune lecture », ne laisse jamais le compteur kilométrique reculer.
- **Traqueur GPS**, plus de 100 entités réparties sur plusieurs plateformes, plus de 20 appels de service, plusieurs véhicules par compte.
...
|---|---|---|---|
| **Audi** | ✅ Bidirectionnel | ✅ Complet | backend myAudi (incl. démarrage/arrêt moteur thermique) |
| **Škoda** | ✅ Bidirectionnel | ✅ Complet | backend Škoda natif |
| **Porsche** | ✅ Bidirectionnel | ✅ Complet | Porsche Connect — backend propre, pas le portail EU Data Act |
| **VW US/CA** | ✅ Bidirectionnel | ✅ Complet | cloud VW NA (nécessite le sélecteur de pays US/CA + S-PIN) |
...
- **Navigateur / code d'appareil (sans mot de passe)** — *Audi · Škoda · SEAT · CUPRA.* Connectez-vous sur votre téléphone ou votre ordinateur portable et approuvez l'appareil ; aucun mot de passe n'est stocké dans Home Assistant (il conserve un vrai jeton de rafraîchissement). Cette étape propose aussi les champs facultatifs **S-PIN**, intervalle de scan et accès forcé.
- **Portail — e-mail + mot de passe** — *Volkswagen EU · Porsche.* Saisissez les identifiants de votre marque. Cette étape expose un sélecteur de marque (Volkswagen EU, Porsche et les autres marques e-mail/mot de passe), e-mail, mot de passe, **S-PIN** facultatif, intervalle de scan, accès forcé et une bascule **« activer les commandes MBB »** (qui n'a d'effet que sur Volkswagen EU — voir [#584](https://github.com/its-me-prash/vwgroup-connect-ha/issues/584)). Pour **Volkswagen US/Canada**, un **sélecteur de pays (US ou CA)** apparaît ici — il ne s'affiche **que** pour cette marque et n'est utilisé par aucune autre.

> Le **portail EU Data Act n'est pas un troisième bouton de connexion.** C'est la stratégie en lecture seule vers laquelle le coordinateur bascule automatiquement, et qui peut en outre être *ajoutée* comme canal de lecture supplémentaire depuis **Configurer → Options**. Il en va de même du canal web `volkswagen.de` (un canal de lecture supplémentaire, uniquement disponible via les Options et activable à la demande).
...
4. **Activez la demande de données continue** pour la voiture concernée. Sans cela, le portail renvoie *aucune demande de données* pour ce VIN et le véhicule apparaît sans aucune lecture.
...
> **Facultatif :** la bascule des Options **`eu_data_act_auto_kickoff`** peut créer automatiquement la demande de données personnalisée de 15 minutes à votre place. Elle est activable à la demande car sa création implique un **abonnement d'un mois sur votre compte VW**, si bien que l'intégration ne le fera pas sans votre accord.

---

## Ce que vous obtenez

- **Capteurs :** SoC de la batterie, autonomie (électrique / thermique / totale), niveau de carburant, compteur kilométrique, températures, puissance/débit/type de charge, cible de charge, statistiques de trajet et cumuls sur la durée de vie, intervalles d'entretien et de vidange, version logicielle, état de connexion, dernière vue, et plus encore.
...
- **Traqueur d'appareil :** position GPS pour la carte de Home Assistant.
...
intervalle de scan, S-PIN, géocodage inverse, **mode lecture seule**, forçage de la climatisation PPE (Audi), bascules de push (MQTT/FCM/Audi-VW), **bascule navigateur EU Data Act** (Playwright / Chromium d'environ 100 Mo, activable à la demande), **réveil avant interrogation** + délai de réveil, surcharge du client-id, **`eu_data_act_auto_kickoff`**, masquer les entités vides (activé par défaut), **ABRP** (activation + api_key + jeton utilisateur, validés par paire), ainsi que l'**ajout / la suppression** des canaux de lecture supplémentaires `volkswagen.de` et portail EU Data Act.
...
Les PR sont les bienvenues — voir [`CONTRIBUTING.md`](CONTRIBUTING.md). Le **Vehicle Data Scout** transforme les champs d'API inconnus en un rapport de bug pré-rempli, en un clic, pour que vous puissiez aider à améliorer la couverture sans lire de code.
