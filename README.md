...
- **GPS device tracker**, 100+ entities across multiple platforms, 20+ service calls, multi-vehicle per account, entity names in **12 languages**.
...
| **VW US/CA** | ✅ Two-way | ✅ Full | VW NA cloud (needs the US/CA country selector + S-PIN). ⚠️ VW's own sign-in service is currently answering some Canadian logins with a server error, which is an outage on VW's side, not a bad password ([#915](https://github.com/its-me-prash/vwgroup-connect-ha/issues/915)) |
...
| **Audi US/CA** | ⚠️ Experimental | ⚠️ Experimental | Login is wired against the NA identity provider but **not yet confirmed working** on a live US/CA account. Testers welcome ([#13](https://github.com/its-me-prash/vwgroup-connect-ha/issues/13)) |
...
- **North America is experimental.** **Audi US/CA** login is wired but has never been confirmed against a live account ([#13](https://github.com/its-me-prash/vwgroup-connect-ha/issues/13)). **VW US/CA** works, but VW's own sign-in service is currently returning server errors for some Canadian accounts ([#915](https://github.com/its-me-prash/vwgroup-connect-ha/issues/915)). Don't pick this integration for a North-American car expecting it to just work.
...
- **Browser / device-code (passwordless)** for *Audi, Škoda, SEAT, CUPRA and Audi US/CA (experimental)*. Sign in on your phone or laptop and approve the device; no password is stored in Home Assistant (it keeps a real refresh token). This step also offers the optional **S-PIN** and scan interval.
- **Portal, email + password** for *Volkswagen EU, Volkswagen US/CA, Bentley and Porsche (experimental)*. Enter your brand login. This step exposes a brand picker, email, password, optional **S-PIN**, scan interval, and an **"enable MBB commands"** toggle (which only has an effect on Volkswagen EU, see [#584](https://github.com/its-me-prash/vwgroup-connect-ha/issues/584)). For **Volkswagen US/Canada** a **country selector (US vs CA)** appears here; it renders **only** for that brand and is not used by any other.
