...
def test_tyre_pressure_diff_sentinel_dropped() -> None:
    # 0=unsupported / 1=invalid → dropped at _walk_fields, never surfaces.
    assert _map_flatlog(
        ("tyre_pressure_differential_front_left", "1")
    ).tyre_pressure_diff_fl is None
    assert _map_flatlog(
        ("tyre_pressure_differential_front_right", "0")
    ).tyre_pressure_diff_fr is None
